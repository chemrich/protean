...
//
// Mol* 5.6.0's ssao.frag,
// {what},
// and nothing else changed. Measured through
...
def branchless_predicate(glsl: str) -> str:
    """5.6.0's ssao.frag with isOutsideBounds itself made branchless.

    Everything measured so far says the cost is neither the jump nor the
    divisor. Reverting the whole skip recovers 90% of the step; keeping the skip
    and restoring the constant divisor recovers nothing (1.164x against stock
    5.6.0's 1.150x); and keeping the test while replacing the jump with a
    multiply is *slower* still (1.204x), because it pays the fetches the jump
    was skipping. What survives all three is the predicate.

    Which is not the seven ALU operations it looks like. **GLSL's `||`
    short-circuits.** The shipped predicate

        coords.x < uBounds.x || coords.x > uBounds.z ||
        coords.y < uBounds.y || coords.y > uBounds.w

    is a chain of up to four conditional evaluations, and on a SIMD rasteriser
    each one is a mask and a jump over a lane group. Four of those, plus the
    skip's own, per sample, 128 samples per fragment, sixteen passes per
    capture. The single-comparison `isBackground` guard beside it costs 2.8%;
    five branches costing about six times that is the right order.

    So this replaces the chain with `any(bvec4(lessThan(...), greaterThan(...)))`
    — the same answer, from vector builtins that do not short-circuit, and one
    branch instead of five. The skip stays exactly as upstream wrote it.

    If the short-circuit chain is the mechanism this recovers most of the step
    with output identical to 5.6.0's, which would make it a repair worth sending
    upstream rather than carrying. If it recovers nothing, the predicate is not
    it either and the remaining suspect is the un-clamping itself.
    """
    before = (
        """bool isOutsideBounds(const in vec2 coords) {
    return coords.x < uBounds.x || coords.x > uBounds.z || """
        + """coords.y < uBounds.y || coords.y > uBounds.w;
}"""
    )
    after = (
        """bool isOutsideBounds(const in vec2 coords) {
    // Same answer, without the short-circuit: GLSL's || is sequential, so the
    // four-term chain this replaces is up to four conditional evaluations per
    // sample. lessThan/greaterThan/any are vector builtins and are not.
    return any(bvec4(lessThan(coords, uBounds.xy), """
        + """greaterThan(coords, uBounds.zw)));
}"""
    )
    if glsl.count(before) != 1:
        raise shader_swap.SwapError(
            f"the bounds predicate is not in the shape this expects "
            f"(found {glsl.count(before)}); Mol* has rewritten it"
        )
    return glsl.replace(before, after, 1)


def flat_predicate_and_mask(glsl: str) -> str:
    """Both at once: the flat predicate AND the jump replaced by a mask.

    The four variants make a 2x2 — {short-circuiting ||, flat} against
    {continue, mask} — and only the full grid can say which axis carries the
    cost. `branchless` is the short-circuit predicate with the jump removed and
    reads *slower* than stock, which on its own looks like "removing the jump is
    bad" and is more likely "the three hidden branches were still there and the
    fetches were no longer being skipped".

    The clamp is kept rather than dropped. Mol* creates every texture with
    CLAMP_TO_EDGE (mol-gl/webgl/texture.js), so for a full-viewport uBounds the
    sampler already does it and the four operations are duplication. They are
    kept anyway because a masked sample is multiplied by zero rather than
    jumped over, and `0.0 * NaN` is NaN: a sample projected through a near-zero
    w would otherwise poison the accumulator, where 5.6.0's continue steps
    around it. Four operations to not have that is the right trade, and it is
    the sort of thing a repair sent upstream has to have thought about.
    """
    return branchless_bounds(branchless_predicate(glsl))
...
VARIANTS = [
    (
        ...
        revert_bounds_skip,
        "with PR #1740 + #1741 reverted -- the isOutsideBounds skip and its\n"
        "// renormalisation -- so that one upstream repair can be priced on its own",
    ),
    (
        "ssao-5.6.0-no-bg-guard.frag",
        revert_background_guard,
        "with PR #1737's shader half reverted -- the opaque isBackground guard",
    ),
    (
        "ssao-5.6.0-pre-1741.frag",
        revert_renormalisation,
        "with PR #1741 alone reverted, so the skip is priced apart from its divisor",
    ),
    (
        "ssao-5.6.0-branchless.frag",
        branchless_bounds,
        "with the jump replaced by a mask -- same behaviour, no per-sample branch",
    ),
    (
        "ssao-5.6.0-flat-predicate.frag",
        branchless_predicate,
        "with the same bounds test written without GLSL's short-circuiting ||",
    ),
    (
        "ssao-5.6.0-flat-and-masked.frag",
        flat_predicate_and_mask,
        "with the flat predicate AND the jump replaced by a mask",
    ),
]
