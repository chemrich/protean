...
# the only thing that separates it from a stall. What a stall no longer has to
# be detected by is a dropped viewer: the bridge now fails in-flight requests
# when the page goes away, so this budget can afford to be generous.
#
...
def _capture_timeout(width: int) -> float:
    ...
    return max(_CAPTURE_TIMEOUT_FLOOR, _CAPTURE_SECONDS_PER_MEGAPIXEL * megapixels)
...
async def snapshot(
    ...
    bridge = _require_viewer()
    timeout = _TRACED_SCREENSHOT_TIMEOUT if _path_tracing else _capture_timeout(width)
