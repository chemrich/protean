...
def _bridges(
    ...
    Every rule reads residues i-1/i+1 and j-1/j+1 as sequence neighbours, so
    both partners need an unbroken peptide run either side of them. Helices,
    turns and bends already guard this; without the same guard here, the last
    residue of one chain acts as "the preceding residue" of the first residue of
    the next, and a real hydrogen bond across that junction is reported as a
